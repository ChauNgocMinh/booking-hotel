document.addEventListener("DOMContentLoaded", () => {
    if (checkRole()) {
        ...
        const userAuth = document.getElementById("showUserAuth");
        if (userAuth) userAuth.classList.remove("d-none");

        const name = document.getElementById("TenDangNhap");
        if (name) name.innerText = localStorage.getItem("Name");

        const role = document.getElementById("userRole");
        if (role) role.innerText = localStorage.getItem("Role");

        if (roleCus()) {
            const avt1 = document.getElementById("userAvt");
            const avt2 = document.getElementById("userAvt2");

            if (avt1) avt1.src = "/images/customer/profile/" + localStorage.getItem("avt");
            if (avt2) avt2.src = "/images/customer/profile/" + localStorage.getItem("avt");

            if (typeof loadNotiList === "function") {
                loadNotiList();
            }
        }
    }
